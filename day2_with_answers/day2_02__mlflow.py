...
# DBTITLE 1,Q1.Answer
# 日毎の集計にするために、timeseriesを加工してsilve tableを作成する
from pyspark.sql.functions import *


# orders silvertable timestamp
order_times = ordersDF.select(to_timestamp(col('order_purchase_timestamp'),'yyyy-MM-dd').alias('time').cast('date'), col('order_id'))

order_times.write \
  .format("delta") \
  .mode("overwrite") \
  .saveAsTable("gd_orders_time")
...
# MAGIC %sql
# MAGIC -- 時系列でorderの状況を可視化
# MAGIC select count(order_id), time from gd_orders_time group by time order by time asc;
...
orderitem_joinDF = orderitemDF.join(ordersDF, ['order_id'], 'inner') \
                              .select(date_format(col('order_purchase_timestamp'),'yyyy-MM-dd').alias('ds').cast('date') \
                                      ,col('price').cast('float') \
                                      ,col('freight_value').cast('float')) \
                              .withColumn('sumprice', col('price') + col('freight_value'))\
                              .groupBy('ds') \
                              .agg(sum('sumprice').alias('y'))

# gold tableとして作成
orderitem_joinDF.write.format('delta') \
                .mode('overwrite') \
                .option("overwriteSchema", "true") \
                .saveAsTable("gd_orderitem_sales")
...
# MAGIC 
# MAGIC -- 売上状況の可視化
# MAGIC select y, ds from gd_orderitem_sales group by ds, y order by ds
...
dbName = f'olist_db_team'   # Database name
