...
-- MAGIC (spark
-- MAGIC   .readStream
-- MAGIC   .format(<<FILL IN>>)                          # Kafkaをソースと指定
...
-- MAGIC   .load()
-- MAGIC   .withColumn(<<FILL IN>>)   # Kafkaのバイナリデータを文字列に変換し、from_json()でJSONをパース
-- MAGIC   .select(col(<<FILL IN>>))                    # JSONの子要素だけを取り出す
-- MAGIC   .writeStream                              # writeStream()でストリームを書き出す
-- MAGIC   .format(<<FILL IN>>)                          # Deltaとして保存
-- MAGIC   .option('checkpointLocation', bronzeCkpt) # チェックポイント保存先を指定
-- MAGIC   .outputMode(<<FILL IN>>)                     # マイクロバッチの結果をAppendで追加
